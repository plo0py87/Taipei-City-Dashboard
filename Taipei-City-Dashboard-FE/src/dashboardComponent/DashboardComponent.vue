...
const dataTime = computed(() => {
	if (props.config.time_from === "static") {
		return i18nStore.$t("components.固定資料");
	} else if (props.config.time_from === "current") {
		return i18nStore.$t("components.即時資料");
	} else if (props.config.time_from === "demo") {
		return i18nStore.$t("components.示範靜態資料");
	} else if (props.config.time_from === "maintain") {
		return i18nStore.$t("components.維護修復中");
	}
...
const updateFreq = computed(() => {
	if (props.config.update_freq && props.config.update_freq_unit) {
		return `${props.config.update_freq}${
			props.config.update_freq_unit
		}${i18nStore.$t("components.更新")}`;
	} else {
		return i18nStore.$t("components.不定期更新");
	}
});
...
<template>
	...
	>
		...
		<div class="dashboardcomponent-header">
			...
			<div>
				...
				<div v-if="!mode.includes('map') || toggleOn">
					<h4
						v-if="
							dataTime === i18nStore.$t('components.維護修復中')
						"
					>
		...
		>
			<span>error</span>
			<p>{{ i18nStore.$t("components.組件資料異常") }}</p>
		</div>
		<div
		...
		>
			...
			>
				<ComponentTag
					...
					:text="i18nStore.$t('components.篩選地圖')"
				...
				<ComponentTag
					...
					:text="i18nStore.$t('components.空間資料')"
					class="hide-if-mobile"
				/>
				<ComponentTag
					v-if="config.history_config?.range"
					:icon="mode === 'preview' ? '' : 'insights'"
					:text="i18nStore.$t('components.歷史資料')"
					class="history-tag"
				/>
			</div>
			<div v-else />
