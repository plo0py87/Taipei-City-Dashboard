...
<template>
	...
	<div v-else-if="contentStore.error" class="dashboard dashboard-nodashboard">
		<div class="dashboard-nodashboard-content">
			...
			<h2>
				{{
					i18nStore.$t("views.DashboardView.發生錯誤，無法載入儀表板")
				}}
			</h2>
		</div>
	</div>
	...
	<div v-else class="dashboard dashboard-nodashboard">
		<div class="dashboard-nodashboard-content">
			<span>addchart</span>
			<h2>{{ i18nStore.$t("views.DashboardView.尚未加入組件") }}</h2>
			<button
				v-if="contentStore.currentDashboard.icon !== 'favorite'"
				class="hide-if-mobile"
				@click="handleOpenSettings"
			>
				{{ i18nStore.$t("views.DashboardView.加入您的第一個組件") }}
			</button>
			<p v-else>
				{{
					i18nStore.$t(
						"views.DashboardView.點擊其他儀表板組件之愛心以新增至收藏組件"
					)
				}}
			</p>
		</div>
