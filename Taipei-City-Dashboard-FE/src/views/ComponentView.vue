...
<template>
	...
	<div class="componentview-search">
		<div>
			<input
				...
				:placeholder="i18nStore.$t('views.ComponentView.以名稱搜尋')"
		...
		<button @click="handleNewQuery">
			{{ i18nStore.$t("views.ComponentView.搜尋") }}
		</button>
	</div>
	...
	<div v-if="contentStore.components.length !== 0" class="componentview">
		<DashboardComponent
			...
			:info-btn-text="i18nStore.$t('views.ComponentView.資訊頁面')"
			@info="
				(item) => {
					router.push({
						name: 'component-info',
	...
	>
		<div class="componentview-nodashboard-content">
			...
			<h2>
				{{ i18nStore.$t("views.ComponentView.發生錯誤，無法載入") }}
			</h2>
		</div>
	</div>
	...
	<div v-else class="componentview componentview-nodashboard">
		<div class="componentview-nodashboard-content">
			...
			<h2>{{ i18nStore.$t("views.ComponentView.查無組件") }}</h2>
			<p>
				{{
					i18nStore.$t("views.ComponentView.請重新搜尋或更改篩選條件")
				}}
			</p>
		</div>
	</div>
